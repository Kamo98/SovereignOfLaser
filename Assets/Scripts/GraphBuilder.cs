...
public class GraphBuilder : MonoBehaviour {
	...
	private Transform[] listOfVertexes;                 //Список вершин графа
	private Dictionary<Transform, int> dictOfVertexes;  //Вершину в число
	...
	void Update()
	{
		for (int j = 0; j < graph.Length; j++)
			foreach (int to in graph[j])
				Debug.DrawLine(listOfVertexes[j].position, listOfVertexes[to].position, Color.white);
	...
	public void rebuild_for_vertex(Transform curVertex, COLOR_OF_VERTEX iteratorOfLaser)
	{
		...
		int j = 0;      //Индекс вершины в списке вершин
		foreach (Transform to in listOfVertexes)
		{
			if (numOfVert != j)
			{				
	...
	private bool raycast_to_vertex(Transform curVertex, Transform to, COLOR_OF_VERTEX iteratorOfLaser)
	{
		...
		foreach (RaycastHit2D hit in hits)
			...
			{
				if (iteratorOfLaser != COLOR_OF_VERTEX.TRANSPARENT)         //КОСТЫЛЬ ДЛЯ КОСТЫЛЯ build_graph, чтобы можно было построить граф
				{
